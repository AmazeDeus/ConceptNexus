...
const ThankYouPage = async ({ searchParams }: PageProps) => {
  ...
  const { docs: orders } = await payload.find({
    collection: 'orders',
    depth: 2, // Query for the whole Product and User. Makes it safe to later cast them as the expected types
    where: {
      id: {
        equals: orderId,
      },
  ...
  if (!order) return notFound(); // 404

  const orderUserId =
    typeof order.user === 'string' ? order.user : order.user.id; // type Order.user: string(id) | User

  // If the attached user is not the same as the logged in user, redirect
  if (orderUserId !== user?.id) {
    return redirect(`/sign-in?origin=thank-you?orderId=${order.id}`);
  }

  const products = order.products as Product[]; // Query depth of 2 guarantees a type of Product[]
            ...
            <div className='mt-16 text-sm font-medium'>
              ...
              <ul className='mt-6 divide-y divide-gray-200 border-t border-gray-200 text-sm font-medium text-muted-foreground'>
                {(order.products as Product[]).map((product) => {
                  // Query depth of 2 guarantees a type of Product[] and ProductFile
              ...
              {/* Poll for payment status on the client side */}
              <PaymentStatus
